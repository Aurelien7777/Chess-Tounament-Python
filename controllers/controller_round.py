...
class RoundController:
    @staticmethod
    def start_round():
        name_round = RoundView.display_name_round()
        date_and_hour_of_start = RoundView.display_date_of_start()
        date_and_hour_of_end = RoundView.display_date_of_end()
        matchs = RoundView.display_matchs()

        round = Round(
            name_round=name_round,
            date_and_hour_of_start=date_and_hour_of_start,
            date_and_hour_of_end=date_and_hour_of_end,
            matchs=matchs,
        )
    ...
    def reconstruire_rounds(name_tournament, joueurs_par_id):
        ...
        for round_tournoi in name_tournament["Informations des tours"]:
            ...
            r = Round(
                matchs=[],
                name_round=round_tournoi["Nom du round"],
                date_of_start=round_tournoi["Date de début"],
                date_of_end=round_tournoi["Date de fin"],
            )

            for match_tournoi in round_tournoi[
                "Matchs"
            ]:  # parcourir les matchs du round
                ...
                match = Match(
                    players=[joueur_0, joueur_1], score=match_tournoi["Score"]
                )
