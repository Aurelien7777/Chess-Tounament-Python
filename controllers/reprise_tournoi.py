...
def resume_tournament():
    ...
    def update_classement_and_available_players(classement_after_round, matches_played_this_round):
        ...
        for gamer in classement_after_round: # parcourir le classement
            if gamer.id not in already_played: # si le joueur n'a pas encore joué dans CE round
                classement_disponible.append(gamer) # ajouter aux joueurs dispo
        if len(classement_disponible) < 2: # pas assez de joueurs dispo
            classement_disponible = classement_after_round[:] # reset si plus assez de joueurs dispo
    ...
    def research_pair_no_played(classement_disponible, matches_played_this_round):
        ...
        while index_de_recherche < len(classement_disponible): # tant que l'index de recherche est inférieur au nombre de joueurs dispo
            search_next_player = index_de_recherche + 1 # chercher le joueur suivant
            while search_next_player < len(classement_disponible): # tant que l'index du joueur suivant est inférieur au nombre de joueurs dispo
                ...
                game_key = tuple(sorted((player_1.id, player_2.id))) # clé du match à tester
                if game_key not in matches_played_this_round: # si la paire n'a pas encore joué dans CE round
                    match_after_round = Match(players=[player_1, player_2], score=[0, 0]) # créer le match
                    matches_played_this_round.append(game_key) # ajouter la paire aux matchs joués dans CE round
                    break
    ...
    def manage_match(match_after_round):
        ...
        # utilisation de key qui permet de trier selon un attribut spécifique déterminé grâce à une fonction lambda. Fonction lambda qui prend un joueur en entrée et renvoie son score.
        classement_after_round = sorted(all_players, key=lambda player_in_classement: player_in_classement.score, reverse=True) 
    ...
    def verify_tournament_completion():
        ...
        if len(data_tournament.list_of_round) >= data_tournament.number_of_round:
            ...
            classement_final = sorted(all_players, key=lambda player_in_classement: player_in_classement.score, reverse=True)
    ...
    def closing_round_normal(round_obj, round_match_list):
        ...
        classement_apres = sorted(all_players, key=lambda player_in_classement: player_in_classement.score, reverse=True)
    ...
    while True:
        ...
        if rebuilding_rounds and len(rebuilding_rounds[-1].matchs) < number_of_match: # si le nombre de matchs joués dans le dernier round est inférieur au nombre de matchs total prévu. Vérification effectuée uniquement si rebuilding_rounds n'est pas vide.
        ...
        for _ in range(remaining_matches):
            ...
            classement_disponible = update_classement_and_available_players(classement_after_round, matches_played_this_round)
            # chercher une paire non jouée dans CE round
            match_after_round = research_pair_no_played(classement_disponible, matches_played_this_round)
