...
def serializer_tournament(obj):
    ...
    if isinstance(obj, Tournament): # Vérification que l'objet de classe crée "obj" est bien du même type que Tournament
        ...
        data_tournament = {"Nom du tournoi": obj.name_of_tournament, 
                        ...
                        "Informations des tours":[round_to_dict(r) for r in obj.list_of_round], # Boucle pour sérialiser chaque round dans la liste des rounds qui contient des objets Match. Informations des tours est une liste de dictionnaires contenant les informations de chaque round. Round qui contiennt des objets Match.
...
def save_data(data_tournament, data_base_tournament):
    ...
    data = serializer_tournament(data_tournament) # Sérialisation de l'objet python Tournament en dictionnaire pour quelle puisse être stockée dans TinyDB.
    
    request_tournament = Query() # L'objet utilisé pour créer les requêtes
    
    request_data_tournament = (request_tournament["Nom du tournoi"] == data["Nom du tournoi"]) # Requête pour vérifier si le tournoi existe déjà dans la base de données TinyDB.
    
    existing = db.get(request_data_tournament) # Vérifie si le tournoi existe déjà dans la base de données TinyDB.
    if not existing: # Si le tournoi n'existe pas encore dans la base de données TinyDB.
        ...
        db.update({"Informations des tours": data["Informations des tours"]}, request_data_tournament) # Mise à jour des informations des tours contenant des objets Match sérialisés.
...
def charger_tournoi_par_nom(data_base_tournament_path, nom_tournoi):
    ...
    object_request_tournoi = Query() # Query est une classe de TinyDB qui permet de construire des requêtes pour interroger la base de données.
    request_tournoi = db.get(object_request_tournoi["Nom du tournoi"] == nom_tournoi) # db.get va rechercher le 1er élément qui correspond à la requête dans le fichier JSON.
    db.close()
