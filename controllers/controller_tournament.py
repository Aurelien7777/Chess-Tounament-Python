...
class TournamentController:
    ...
    def create_tournament():
        ...
        name_of_tournament = TournamentView.display_name_of_tournament().strip()
        place = TournamentView.display_place().strip()
        date_of_start = TournamentView.display_date_of_start().strip()
        date_of_end = TournamentView.display_date_of_end().strip()
        description = TournamentView.display_description().strip()

        while (
            not name_of_tournament or not place or not date_of_start or not date_of_end
        ):
            print(
    ...
    def save_data(data_tournament, data_base_tournament):
        ...
        data = TournamentController.serializer_tournament(
            data_tournament
        ...
        if (
            ...
            db.insert(
                data
            )  # Insère les données du tournoi dans la base de données TinyDB.
            TournamentSaveView.display_save_tournament(data)

        else:
            db.update(
                {"Liste des joueurs": data["Liste des joueurs"]},
                request_data_tournament,
            )
            db.update(
            ...
            db.update(
                {"Lieu du tournoi": data["Lieu du tournoi"]}, request_data_tournament
            )
            db.update(
            ...
            db.update(
                {"Nombre de round": data["Nombre de round"]}, request_data_tournament
            )
