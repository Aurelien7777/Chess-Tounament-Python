...
class MatchController:
    ...
    def create_match(list_player, matches_played):
        ...
        if len(list_match_player) == 2:
            ...
            match_key = tuple(
                sorted((list_match_player[0].id, list_match_player[1].id))
            )

            if match_key in matches_played:
                return MatchController.create_match(list_player, matches_played)
            else:
    ...
    def match_after_first_round(classement_after_round, matches_played_round):
        ...
        for i in range(len(classement_after_round)):
            premier_du_classement = classement_after_round[
                i
            ]  # Sélection du joueur possédant le meilleur classement
            for j in range(
                i + 1,
                len(
                    classement_after_round
                ),  # Sélection du joueur possédant le 2ème meilleur classement
            ):  # Je prends le joueur suivant dans le classement
                deuxieme_meilleur = classement_after_round[
                    j
                ]  # Enregistrement du joueur possédant le 2ème meilleur classement

                # Clé de la paire (ordre neutre)
                match_key = tuple(
                    sorted((premier_du_classement.id, deuxieme_meilleur.id))
                )

                # Vérifie si la paire n'a jamais joué dans ce round
                if (
                    match_key not in matches_played_round
                ):  # Si la paire n'a pas encore joué ensemble
                    matches_played_round.append(
                        match_key
                    )  # Ajout de la paire aux matchs joués dans le round
    ...
    def choice_white_or_black(list_match_player):
        ...
        MatchResultView.display_white_player(list_match_player, player_start)
    ...
    def manage_winner_match_bis(list_match_player):
        ...
        while winner == -1 or winner > 2:
            try:
                winner = MatchResultView.choice_winner_match()
            except ValueError:
                MatchResultView.display_input_result_of_match_invalid()
                return MatchController.manage_winner_match_bis(list_match_player)
    ...
    def classement(winner_list, draw_list, looser_list):
        ...
        for player_classement_final in classement_after_match:
            MatchResultView.display_classement_final(compteur, player_classement_final)
            compteur += 1
