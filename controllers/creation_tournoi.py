...
def lancer_creation_tournoi():
    ...
    def creation_joueurs_tournoi(tournament, number_player_in_tournamment):
        ...
        return save_players(tournament.list_player_saved, "data_base_players.json") # Enregistrement des joueurs dans le fichier JSON

    # CREATION DE L'OBJET ROUND
    def creation_objet_round(tour):
        # Création de l'objet Round
        return Round(matchs=[], name_round=f"Round {tour+1}",
    ...
    for tour in range(1): 
        ...
        for _ in range(number_of_match // 2):
            ...
            # Sélection aléatoire de 2 joueurs dans la copie de la liste "short_lived_list" qui contient tous les joueurs
    ...
    for tour in range(tournament.number_of_round - 1): #Execution du second round
        ...
        for _ in range(number_of_match // 2):
            ...
            # GESTION DES GAGNANT/PERDANT/MATCH NUL
                resultat_after_first_round = gestion_gagnant_perdant(match_after_round, tournament, winner_list, draw_list, looser_list)
