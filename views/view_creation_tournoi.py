...
def display_no_possible_match():
    print(
        "Aucun match possible (plus assez de joueurs ou plus de paires disponibles).\n"
    )


def display_match_information(match):
    print(
        f"Le match est {match.players[0].name} {match.players[0].last_name} "
        f"contre {match.players[1].name} {match.players[1].last_name}\n"
    )
...
def request_number_of_players():
    ...
    return int(
        input("Combien de joueurs participe au tournoi? ")
